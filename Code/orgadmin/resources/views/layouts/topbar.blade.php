    ...
    <div class="flex items-center justify-between">
      <div class="flex items-center justify-start rtl:justify-end">
        ...
        <a href="{{ route('dashboard') }}" class="flex ms-2 md:me-24">
          ...
          <span
            class="self-center text-lg font-semibold whitespace-nowrap text-heading">{{ config('app.name', 'App') }}</span>
        </a>
      </div>
          ...
          <div>
            <button type="button"
              ...
              <img class="w-8 h-8 rounded-full"
                src="{{ auth()->user()->profile_photo_url ?? 'https://ui-avatars.com/api/?name=' . urlencode(auth()->user()->name ?? 'User') . '&background=0D8ABC&color=fff' }}"
